...
<html>
    ...
    <body>
        <nav class="navbar navbar-expand-md" style="background-color: #FF8C00; position: fixed; width: 100%; z-index: 1;">
            <a class="navbar-brand" href="InitSearchPageServlet" id="aHeader">The Book Store</a>
            <c:if test="${empty sessionScope.USER}">
                <a id="aLogin" class="nav-link" href="login.jsp">Login</a>
            </c:if>
            <c:if test="${not empty sessionScope.USER}">
                <li id="liUser" class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle" id="navbardrop" data-toggle="dropdown" style="color: white; font-weight: bold;">
                      Welcome, ${sessionScope.USER}
                    </a>
                    <div class="dropdown-menu">
                    <c:if test="${not empty sessionScope.ADMIN}">
                        <c:if test="${!sessionScope.ADMIN}">
                             <a class="dropdown-item" href="ServletDispatcher?btAction=History">History</a>
                        </c:if>
                        <a class="dropdown-item" href="ServletDispatcher?btAction=Logout">Logout</a>
                    </c:if>
                    </div>
                </li>
            </c:if>   
        ...
        <c:if test="${not empty sessionScope.USER}">
            <c:if test="${sessionScope.ADMIN}">
                <div class="pl-3" style="display:flex;">
                    <form action="createNewBook.jsp" style="width: 10%;">
                        <input 
                            class="btn shadow-sm" 
                    ...
                    <br/>
                    <form action="ServletDispatcher" style="width: 90%;">
                        <input 
                            class="btn shadow-sm" 
                            style="background-color: #FF8C00; color: #F0F8FF;" 
        ...
        <div class="pl-3" style="font-size: 1.5rem; font-weight: bold;">
            Search Page 
        </div>
        <br/>
        <div class="pl-3" style="display: flex;">
            <form class="form-inline" action="ServletDispatcher" style="width: 50%;">
            ...
            <form action="ServletDispatcher" style="width: 50%;">
                <c:set var="categories" value="${sessionScope.CATEGORIES}" />
                <c:if test="${not empty categories}">
                    <div class="input-group mb-3" style="padding-right: 60%;">
                        <div class="input-group-prepend shadow-sm">
                            <label class="input-group-text" for="categorySelect">Category</label>
                        </div>
                        <select class="custom-select shadow-sm" name="category" id="categorySelect" style="width: 40%;">
                            <option value="All"
        ...
        <br/>
        <form class="form-inline" action="ServletDispatcher" style="margin-left: 450px;">
            <div class="pl-3">
                        ...
                        <c:forEach var="dto" items="${result}" varStatus="counter">
                            ...
                            <c:if test="${not empty sessionScope.ADMIN}">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <img class="rounded" src="image/${dto.image}" alt="No Image" height="200px" width="150px" />
                                    ...
                                    <c:if test="${not empty sessionScope.ADMIN}">
                                        <c:if test="${sessionScope.ADMIN}">
                                            ...
                                            <td>
                                                <c:url var="deleleLink" value="ServletDispatcher">
